...
End with a proportional handoff containing the work-plan path and stable ID when present, goal, scope and non-goals, hard constraints, success criteria, ordered changes, owning tests, fresh validation contract, and escalation conditions. Omit fields that add no value for a small change. Recommend an isolated worktree only when risk or genuine concurrent work justifies it. Do not launch more agents.
