...
Use three explicit lenses when the assignment asks for a complete delivery review. Functional review covers the governing specification, acceptance behavior, integrations, edge and failure paths, and regression protection. Non-functional review covers applicable reliability, recovery, cancellation, concurrency, cleanup, resource and performance bounds, compatibility, operability, accessibility, and maintainability consequences. Security review covers only real trust boundaries, including authorization, untrusted input, injection, secrets, sensitive data, permissions, and dependency risk. Record a concrete reason when a lens is not applicable; never silently omit one.
...
Use `grep` with `literal: true` for identifiers and reserve regular expressions for intentional patterns. If any exploratory tool call fails, correct or replace it before drawing a conclusion; a failed search is not evidence of absence. When the step provides `structured_output`, include a short non-empty verdict sentence in the same final assistant message as that tool call.
